...
def get_positive_features(train_path_pos, feature_params):
    ...
    features_pos = []
    for filename in glob(train_path_pos + '/*.jpg'):
        img = imread(filename)
        img_mirror = img[:, ::-1]
        hog_features = hog(img)
        hog_features_mirror = hog(img_mirror)
...
def get_random_negative_features(non_face_scn_path, feature_params, num_samples):
    ...
    i = 0
    while i < num_samples:
        for filename in glob(non_face_scn_path + '/*.jpg'):
            if i >= num_samples:
                break
            img = imread(filename)
